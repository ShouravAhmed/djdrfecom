...
class PathaoApi(object):
    def __init__(self, client_id=None, client_secret=None, username=None, password=None, base_url=None):
        self.base_url = base_url or os.environ.get('PATHAO_API_BASE_URL')
        self.client_id = client_id or os.environ.get('PATHAO_API_CLIENT_ID')
        self.client_secret = client_secret or os.environ.get(
            'PATHAO_API_CLIENT_SECRET')
        self.username = username or os.environ.get('PATHAO_API_USERNAME')
        self.password = password or os.environ.get('PATHAO_API_PASSWORD')
    ...
    def get_response_data(self, url, data=None, method='get', headers=None):
        ...
        if method == 'post':
            response = requests.post(
                url, json=data, headers=headers or self.auth_headers)
    ...
    def create_order(self, store_id, order_id, sender_name, sender_phone, recipient_name, recipient_phone, address,
        ...
        data = self.get_response_data(
            url=url, data=data, method='post', headers=self.auth_headers)
        return data
    ...
    def get_delivery_cost(self, store_id, city_id, zone_id, item_type='2', delivery_type=48, item_weight=.5):
        ...
        data = {'store_id': store_id,
                ...
                'recipient_zone': zone_id}
        data = self.get_response_data(
            url, data=data, method='post', headers=self.auth_headers)
        return data
